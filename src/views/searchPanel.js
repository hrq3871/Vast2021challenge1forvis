import { Search, RotateCcw, X } from 'lucide';
...
export function renderTopBar(container, state, bundle) {
  ...
  container.innerHTML = `
    <div class="brand-block">
      ...
      <h1>Relationship Workbench</h1>
    </div>
    <label class="search-field">
      <span class="sr-only">Search entities and evidence</span>
      ${iconSvg(Search)}
      <input id="global-search" type="search" placeholder="Search Sanjorge, Vann, kidnapping, ARISE" autocomplete="off" />
...
export function renderLeftRail(container, state) {
  const views = [
    ['overview', 'Overview', 'Network'],
    ['official', 'Official', 'Govt'],
    ['pok', 'POK Motive', 'POK'],
    ['email', 'Email Network', 'Mail'],
    ['hypothesis', 'Hypotheses', 'Paths'],
  ];
    ...
    .map(
      ([id, label, short]) => `
        <button class="rail-button" type="button" data-view="${id}" aria-pressed="false">
          <span class="rail-short">${short}</span>
          <span>${label}</span>
        </button>
      `,
    )
